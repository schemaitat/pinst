...
- [ ] TASK-032: `scripts/install.sh` (FILE-018) — replace the single `TARGET`
...
- [ ] TASK-033: `scripts/install.sh` (FILE-018) — have `download_release()`
...
- [ ] TASK-034: `scripts/install.sh` (FILE-018) — confirm the source-build
...
- [ ] TASK-035: `.github/workflows/ci.yml` (FILE-017) — make the `check` job
...
- [ ] TASK-036: `.github/workflows/ci.yml` (FILE-017) — add a step on the
...
- [ ] TASK-037: `README.md` (FILE-019) — state the supported platforms, what
...
*(To be filled in by TASK-038 — record the macOS version, the architecture,
which tools failed to install if any, and what `pinst doctor` reported at the
end. Until this section has content, the phase stays In Progress.)*
