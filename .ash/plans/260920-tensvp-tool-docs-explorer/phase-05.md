...
## Steps
- [x] TASK-027: seed every capture-capable tool with `pinst docs adopt`, so
      authoring starts from the tool's real help on this machine rather than
      from memory of what its flags are.
- [x] TASK-028: author the pages for the tools an agent reaches for daily —
...
- [x] TASK-029: author the pages for the local tools no model has priors
...
- [x] TASK-030: write short pages for the tools with no CLI surface —
      ...
      unwritten page.
- [x] TASK-031: query the finished catalogue with the intentions an agent
...
- [x] TASK-032: measure and record the cost: `just dist` before and after the
